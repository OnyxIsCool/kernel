--[[
	...
	Dex++ [SECURED] is a revival and secured version of Moon's Dex, made to fulfill Moon's Dex prophecy.
]]--
...
local function ScanObj(obj)
    pcall(function()
        safeLog("Dex Object:", obj.ClassName, obj:GetFullName())

        if obj:IsA("RemoteEvent") or obj:IsA("RemoteFunction") then
            safeLog("Remote Found:", obj:GetFullName())
        end
    end)
end
...
LogService.MessageOut:Connect(function(message, messageType)
    pcall(function()
        safeLog("Log:", messageType, message)

        local lower = string.lower(message)
        if lower:find("kick") or lower:find("ban") or lower:find("mimic") or lower:find("freeze") or lower:find("crash") then
            safeLog("WARNING! Dex Detected an Keyword in LogService:", message)
        end
    end)
end)

--This is for the mimic and other ContentProvider
local oldPreloadAsync
oldPreloadAsync = hookfunction(ContentProvider.PreloadAsync, function(self, assets, ...)
    pcall(function()
        safeLog("ContentProvider PreloadAsync called")

        if tostring(assets):lower():find("mimic") then
            safeLog("WARNING! The Mimic asset detected")
        end
    end)

    return oldPreloadAsync(self, assets, ...)
end)
...
if mt then
    ...
    mt.__namecall = newcclosure(function(self, ...)
        ...
        and (self:IsA("RemoteEvent") or self:IsA("RemoteFunction")) then
            pcall(function()
                safeLog("Remote Call:", self:GetFullName(), "Method:", method)
                for i,v in ipairs(args) do
                    safeLog(" Arg["..i.."] =", tostring(v))
                end
            end)
        end
        ...
        if method == "Kick" and self == LocalPlayer then
            ...
            safeLog("DEX ALERT! Kick attempt blocked:", reason)
            notify("The game tried to block or kick you\nBut dex prevented it.\nReason: "..tostring(reason))
            return nil
        end
...
LocalPlayer.Kicked:Connect(function(reason)
    safeLog("Kick event detected:", reason)
    notify("The game tried to block or kick you\nBut dex prevented it.\nReason: "..tostring(reason))
end)

LocalPlayer.OnTeleport:Connect(function(state)
    safeLog("Teleport State:", state)
end)

--Scanner Function
task.spawn(function()
    while task.wait(10) do
        safeLog("Scanning everything...")
    end
end)
print("Dex Protection Scanner Activated✅")
--End of the scanner code

--// Thanks TTJY For this code
...
if not GG.ReplicatedFirst_lc then
    GG.setc = setclipboard or toclipboard or function() end
    local dbg = debug or {}
    local gi = getinfo or dbg.getinfo
    if not gi then return end
    local cf = clonefunction or clonefunc or function(f) return f end
    local cr = cloneref or clonereference or function(r) return r end
    if gi(cf).what == "Lua" then cf = function(...) return ... end end
    if gi(cr).what == "Lua" then cr = function(...) return ... end end
    GG.clonefunction = cf
    GG.cloneref = cr
    GG.base64encode = base64encode or (crypt and crypt.base64encode)
    GG.base64decode = base64decode or (crypt and crypt.base64decode)
    GG.GetService = cf(game.GetService)
    GG.getinfo = cf(gi)
    GG.pcall = cf(pcall)
    GG.tble = table
    GG.tk = task
    ...
    GG.bff = buffer
    GG.SecureEnv = {
        dumpheap = dumpheap or dbg.dumpheap,
        getconstants = getconstants or dbg.getconstants,
        getproto = getproto or dbg.getproto,
        setmemorycategory = setmemorycategory or dbg.setmemorycategory,
        profilebegin = profilebegin or dbg.profilebegin,
        loadmodule = loadmodule or dbg.loadmodule,
        traceback = traceback or dbg.traceback,
        getinfo = gi,
        getstack = getstack or dbg.getstack,
        isvalidlevel = isvalidlevel or dbg.isvalidlevel,
        getupvalues = getupvalues or dbg.getupvalues,
        getconstant = getconstant or dbg.getconstant,
        getfenv = getfenv or dbg.getfenv,
        getupvalue = getupvalue or dbg.getupvalue,
        getmemorycategory = getmemorycategory or dbg.getmemorycategory,
        resetmemorycategory = resetmemorycategory or dbg.resetmemorycategory,
        getprotos = getprotos or dbg.getprotos,
        dumpcodesize = dumpcodesize or dbg.dumpcodesize,
        setstack = setstack or dbg.setstack,
        profileend = profileend or dbg.profileend,
        dumprefs = dumprefs or dbg.dumprefs,
        validlevel = validlevel or dbg.validlevel,
        setupvalue = setupvalue or dbg.setupvalue,
        setconstant = setconstant or dbg.setconstant,
        getregistry = getregistry or dbg.getregistry,
        info = info or dbg.info,
        wait = wait,
        delay = delay,
        spawn = spawn,
        tick = tick,
        ...
        tcancel = task.cancel,
        tdelay = task.delay,
        tdesyn = task.desynchronize,
        tsyn = task.synchronize,
        strgsub = string.gsub,
        strsub = string.sub,
        strfind = string.find,
        strlen = string.len,
        ...
        strpack = string.pack,
        strpacksize = string.packsize,
        strreverse = string.reverse,
        strunpack = string.unpack,
        strrep = string.rep,
        mlog = math.log,
        mldexp = math.ldexp,
        mdeg = math.deg,
...
if getgenv().DEX_SECUREDLOADED then return end
getgenv().DEX_SECUREDLOADED = true
local selection = nil
local nodes = {}
...
local function loadModule(name)
    if EmbeddedModules[name] then
        local ok, result = pcall(EmbeddedModules[name])
        if ok then
            return result
        end
    end
...
local ConsoleModule = loadModule("Console")

local function setSelection(obj)
    if typeof(obj) == "Instance" then
        selection = obj
    end
end

local function trackNode(obj)
